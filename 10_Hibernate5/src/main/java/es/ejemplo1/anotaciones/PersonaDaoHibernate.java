...
public class PersonaDaoHibernate implements PersonaDao{
    ...
    private Session iniciarSession(){
        ...
        //Nota, en aplicaciones web mejor utilizar el metodo getCurrentSession()
        //Este metodo asocia una sesion al hilo de ejecución que se encuentra
    	return session;
    }
    ...
    public void borrar(Persona persona) throws HibernateException{        
    	try(Session session = iniciarSession()){
    		...
    		//cacheado, podemos usar delete o remove
    ...
    public Persona get(int i) throws HibernateException{
    	try(Session session = iniciarSession()){
            Persona persona = session.get(Persona.class,i);
            return persona;
        }catch(HibernateException he){
